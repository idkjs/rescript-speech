...
let result = switch Random.int(100) {
| a if a < 5 => "undershoot"
| 5 => "hit the target"
| _ => "overshoot"
}
...
let rec qsort = a =>
  ...
  | list{} => list{}
  | list{x, ...tail} =>
    let (p1, p2) = List.fold_left(
      ((a1, a2), e) => e > x ? (list{e, ...a1}, a2) : (a1, list{e, ...a2}),
      (list{}, list{}),
      tail,
    )
    List.append(qsort(p1), list{x, ...qsort(p2)})
  }

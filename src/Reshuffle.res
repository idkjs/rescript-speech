...
let reshuffle3 = cards =>
  map(((_, x)) => x, sort(((x, _), (y, _)) => x - y, map(x => (Random.int(1000000), x), cards)))
/*  */
/*  */
/*  */
/*  */
...
let \"+>" = (x, f) =>
  f(x) /* x +> f  :  f(x)  -  применение функции к аргументу */
let reshuffle5 = cards =>
  \"+>"(
    \"+>"(\"+>"(cards, map(x => (Random.int(1000000), x))), sort(((x, _), (y, _)) => x - y)),
    map(((_, x)) => x),
  )
...
let reshuffle9 = cards =>
  \"@@"(
    map(((_, x)) => x),
    \"@@"(sort(((x, _), (y, _)) => x - y), \"@@"(map(x => (Random.int(1000000), x)), cards)),
  )
